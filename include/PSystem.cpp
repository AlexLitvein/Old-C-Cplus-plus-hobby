	   ...
	   mMaxNumParticles(maxNumParticles), mTimePerParticle(timePerParticle)
{
	...
	// Create the texture.
	HR(D3DXCreateTextureFromFile(g_pD3Device, texName.c_str(), &mTex));

	// Create the FX.
	ID3DXBuffer* errors = 0;
	HR(D3DXCreateEffectFromFile(g_pD3Device, fxName.c_str(),
	...
	HR(g_pD3Device->CreateVertexBuffer(mMaxNumParticles*sizeof(Particle),
...
void PSystem::draw()
{
	...
	HR(g_pD3Device->SetStreamSource(0, mVB, 0, sizeof(Particle)));
	HR(g_pD3Device->SetVertexDeclaration(Particle::Decl));
	...
	if( gCamera->isVisible( boxWorld ) )
	{
		...
		if(vbIndex > 0)
		{
			HR(g_pD3Device->DrawPrimitive(D3DPT_POINTLIST, 0, vbIndex));
		}
	}
