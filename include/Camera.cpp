...
void Camera::setType(bool bFly) { mFly = bFly; }
...
{	
	...
	//{
		...
		if( gDInput->keyDown(DIK_W) ) dir += mLookW;
		if( gDInput->keyDown(DIK_S) ) dir -= mLookW;
		if( gDInput->keyDown(DIK_D) ) dir += mRightW;
		if( gDInput->keyDown(DIK_A) ) dir -= mRightW;
		...
		//mPosW.y = g_pMyTerrain->GetHeight(mPosW.x, mPosW.z) + offsetHeight;
	...
	msg.type = MAKEDWORD(CAMERA_OBJ, 0);	
	msg.pt  = mPosW;
	gQueueMngr->SendMsgIN(msg);
	//Write_Log_String(hLog, "Camera::update t = %f x = %10f z = %10f\n", g_time, mPosW.x, mPosW.z); 
}
