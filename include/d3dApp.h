...
public:
	...
	HWND      getMainWnd();
	size_t		GetBackBuffWidth() { return md3dPP.BackBufferWidth; }
	size_t		GetBackBuffHeight() { return md3dPP.BackBufferHeight; }

	// Framework methods.  Derived client class overrides these methods to 
	// implement specific application requirements.
	virtual bool checkDeviceCaps()     { return true; }
	
	// Recall that resources in the D3DPOOL_DEFAULT memory pool need to be released 
	// prior to invoking the Reset method, and need to be restored after invoking the Reset method. 
	// For resources in D3DPOOL_DEFAULT, we put the resource release code in onLostDevice and the 
	// resource restoration code in onResetDevice. 
